...
try:
  ...
  for username in usernames:
    ...
    with open('./profiles/{}_post.csv'.format(username), "w") as f:
        f.write(', '.join(information.get("posts")[0].keys())+'\n')

        for post in information.get("posts"):
            f.write(', '.join(str(i) for i in post.values())+'\n')
